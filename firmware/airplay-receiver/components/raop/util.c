...
in_addr_t get_localhost(char **name)
{
    ...
    if (!netif || esp_netif_get_ip_info(netif, &info) != ESP_OK) return INADDR_ANY;
    if (name) {
        const char *host = "volvo-airplay";
        esp_netif_get_hostname(netif, &host);
        *name = strdup(host ? host : "volvo-airplay");
    }
    return info.ip.addr;
