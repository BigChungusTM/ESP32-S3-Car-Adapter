...
static int raop_rng(void *context, unsigned char *output, size_t length) {
    (void)context;
...
struct raop_ctx_s *raop_create(uint32_t host, char *name,
	...
	ctx->thread = xTaskCreateStaticPinnedToCore( (TaskFunction_t) rtsp_thread, "RTSP", RTSP_STACK_SIZE, ctx, 
												 ESP_TASK_PRIO_MIN + 2, ctx->xStack, ctx->xTaskBuffer, RAOP_TASK_CORE);
#endif
...
static char *rsa_apply(unsigned char *input, int inlen, int *outlen, int mode)
{
    ...
    *outlen = 0;
    int rc = mbedtls_pk_parse_key(&pkctx, (const unsigned char *)super_secret_key,
                                 sizeof(super_secret_key), NULL, 0, raop_rng, NULL);
    if (rc != 0) { mbedtls_pk_free(&pkctx); return NULL; }
    mbedtls_rsa_context *rsa = mbedtls_pk_rsa(pkctx);
    size_t length = mbedtls_rsa_get_len(rsa), written = 0;
    unsigned char *output = malloc(length);
    if (!output) { mbedtls_pk_free(&pkctx); return NULL; }
    if (mode == RSA_MODE_AUTH) {
        mbedtls_rsa_set_padding(rsa, MBEDTLS_RSA_PKCS_V15, MBEDTLS_MD_NONE);
        rc = mbedtls_rsa_rsassa_pkcs1_v15_sign(rsa, raop_rng, NULL,
                    MBEDTLS_MD_NONE, inlen, (const unsigned char *)input, output);
        written = length;
    } else {
        mbedtls_rsa_set_padding(rsa, MBEDTLS_RSA_PKCS_V21, MBEDTLS_MD_SHA1);
        rc = mbedtls_rsa_pkcs1_decrypt(rsa, raop_rng, NULL, &written,
                    (const unsigned char *)input, output, length);
    }
    mbedtls_pk_free(&pkctx);
    if (rc != 0) { free(output); return NULL; }
    *outlen = written;
    return (char *)output;
#endif
}
